...
- ระบบสต็อกสินค้า: เปิด/ปิดการตัดสต็อกต่อเมนู, ตัดสต็อกอัตโนมัติเมื่อมีออเดอร์ (กันขายเกินสต็อกแม้มีคนสั่งพร้อมกัน), คืนสต็อกอัตโนมัติเมื่อยกเลิกออเดอร์, เมนูที่หมดจะขึ้น "หมด" ให้ลูกค้าเห็นทันที, หน้าคลังสินค้าไว้เติมสต็อกพร้อมประวัติการเคลื่อนไหว
...
ค่าเริ่มต้นทุกเมนูจะ**ไม่ตัดสต็อก** (ขายได้ไม่จำกัด) เหมาะกับเมนูทำสดไม่มีวันหมด ถ้าอยากติดตามสต็อกของเมนูไหน (เช่น เค้ก, ของหวานจำนวนจำกัด, เครื่องดื่มนำเข้า):

1. ไปที่ **จัดการเมนู** (`/admin/menu`) ติ๊ก "ตัดสต็อก" ที่เมนูนั้น แล้วใส่จำนวนเริ่มต้น กด "บันทึก"
2. ทุกครั้งที่มีออเดอร์ (จากลูกค้าสแกน QR หรือพนักงานเพิ่มเอง) สต็อกจะลดลงอัตโนมัติ ถ้าสต็อกไม่พอจะไม่ให้สั่งเมนูนั้นเกิน
3. เมนูที่สต็อกหมด (`0`) จะขึ้น "หมด" ให้ลูกค้าเห็นทันทีที่หน้าสั่งอาหาร (อัปเดตสดทุก 5 วินาที)
4. ถ้ายกเลิกออเดอร์ สต็อกจะถูกคืนกลับอัตโนมัติ
5. ไปเติมสต็อกหรือดูประวัติการเคลื่อนไหวได้ที่ **คลังสินค้า** (`/admin/stock`)
